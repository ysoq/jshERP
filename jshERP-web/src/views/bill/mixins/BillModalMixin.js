...
export const BillModalMixin = {
  ...
  methods: {
    ...
    onValueChange(event) {
      ...
      switch (column.key) {
        case 'currentStock':
          const stock = row.currentStock - row.stock
          row.operNumber = stock
          target.setValues([{ rowKey: row.id, values: { operNumber: stock } }])
          this.onValueChange({ ...event, column: { ...column, key: 'operNumber' } })
          break
        case 'depotId':
        ...
        case 'taxLastMoney':
          ...
          target.setValues([{
            rowKey: row.id,
            values: { unitPrice: unitPrice, allPrice: allPrice, taxMoney: taxMoney }
          }])
          target.recalcAllStatisticsColumns()
          that.autoChangePrice(target)
          break
