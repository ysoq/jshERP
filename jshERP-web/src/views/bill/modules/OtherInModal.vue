    ...
    <a-spin :spinning='confirmLoading'>
      <a-form :form='form'>
        <a-row class='form-row' :gutter='24'>
          ...
          <a-col :lg='6' :md='12' :sm='24'>
            <a-form-item v-if='inOutManageFlag && !model.billType' :labelCol='labelCol' :wrapperCol='wrapperCol'
                         label='关联单据'>
              <a-input-search placeholder='请选择待入库单据' v-decorator="[ 'linkNumber' ]" @search='onSearchLinkNumber'
                              :readOnly='true' />
            </a-form-item>
          </a-col>
        </a-row>
        <j-editable-table id='billModal'
          ...
          <template #buttonAfter>
            <a-row v-if='rowCanEdit' :gutter='24' style='float:left;' data-step='4' data-title='扫码录入'
  ...
  methods: {
      ...
      if (this.action === 'add') {
        ...
        await this.requestSubTableData(url, params, this.materialTable).then(list => {
          ...
          for (const item of list) {
            item.byInOutItemId = item.inOutItemId
          }
    ...
    classifyIntoFormData(allValues) {
      ...
      for (let item of detailArr) {
        ...
        if (billMain.inOutItemId && item.byInOutItemId && `${item.byInOutItemId}` !== billMain.inOutItemId) {
          this.$message.error(`${item.name}与选择的项目不一致`)
          throw new Error('')
        }
